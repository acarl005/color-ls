...
(function() {
  var BG, BW, _, _s, ansi, args, bold, colors, dirString, extString, fg, fgc, filestats, fs, fw, groupName, j, len, linkString, listDir, listFiles, log, log_error, mediaTypes, months, nameString, ownerName, ownerString, p, path, pathstats, prof, ref, ref1, reset, rightsString, rwxString, since, sizeString, sort, sprintf, start, stats, timeString, token, username,
  ...
  nameString = function(name, ext, stat) {
    var key;
    key = mediaTypes.has(ext) ? "_media" : ext;
    name = " " + colors[(colors[key] != null) && key || '_default'][0] + name + reset;
    if (((stat.mode >> 6) & 0x1) && args.icons) {
      return ' ' + BW(1) + fg(0, 5, 0) + '>_' + reset + name;
    } else {
      return name;
    }
  };
  ...
  listFiles = function(p, files) {
    ...
    files.forEach(function(rp) {
      var ext, file, groupColor, grp, link, lstat, name, own, ownerColor, s, stat;
      if (rp[0] === '/') {
        file = path.resolve(rp);
      } else {
        file = path.join(p, rp);
